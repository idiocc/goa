...
class sc extends ua{constructor(){super();this.silent=this.proxy=!1;this.middleware=[];this.subdomainOffset=2;this.env=process.env.NODE_ENV||"development";this.context=Object.create(Y.prototype);this.request=Object.create(nc.prototype);this.response=Object.create(oc.prototype);this.keys=void 0}[p.custom](){return this.inspect()}listen(...a){Z("listen");return ta(this.callback()).listen(...a)}toJSON(){return{subdomainOffset:this.subdomainOffset,proxy:this.proxy,env:this.env}}inspect(){return this.toJSON()}use(a){if("function"!=
typeof a)throw new TypeError("middleware must be a function!");if("function"!=typeof a?0:qa.test(pa.call(a))||(z?A(a)==ra:"[object GeneratorFunction]"==oa.call(a)))throw Error("Generator functions are not supported by @goa/koa. Use koa-convert on them first.");Z("use %s",a.D||a.name||"-");this.middleware.push(a);return this}callback(){const a=pc(this.middleware);if(!this.listenerCount("error"))this.on("error",this.onerror);return(b,c)=>{b=this.createContext(b,c);return qc(b,a)}}createContext(a,b){const c=
