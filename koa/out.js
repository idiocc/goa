...
function pc(a){if(0!=a.respond&&a.writable){var {res:b,status:c}=a,d=a.body;if(I[c])return a.body=null,b.end();if("HEAD"==a.method)return!b.headersSent&&fb(d)&&(a.length=Buffer.byteLength(JSON.stringify(d))),b.end();if(null==d)return d=2<=a.req.httpVersionMajor?String(c):a.message||String(c),b.headersSent||(a.type="text",a.length=Buffer.byteLength(d)),b.end(d);if(Buffer.isBuffer(d))return b.end(d);if("string"==typeof d)return b.end(d);if(d instanceof D)return d.pipe(b);d=JSON.stringify(d);b.headersSent||
(a.length=Buffer.byteLength(d));b.end(d)}};const rc=new qc;async function sc(a){a.body="hello world";a.set("agc","def")}module.exports=sc;rc.use(sc);rc.listen(3001,()=>{console.log("http://localhost:3001")});
