...
*/
const R={};
class pb{constructor(a,b,c){this.secure=void 0;this.request=a;this.a=b;c&&(this.keys=Array.isArray(c.keys)?new lb(c.keys):c.keys,this.secure=c.secure)}get(a,b){var c=a+".sig",d,e=b&&void 0!==b.signed?b.signed:!!this.keys;if(d=this.request.headers.cookie)if(d=d.match(R[a]?R[a]:R[a]=new RegExp("(?:^|;) *"+a.replace(/[-[\]{}()*+?.,\\^$|#\s]/g,"\\$&")+"=([^;]*)"))){d=d[1];if(!b||!e)return d;if(b=this.get(c)){a=a+"="+d;if(!this.keys)throw Error(".keys required for signed cookies");b=this.keys.index(a,b);
if(0>b)this.set(c,null,{path:"/",signed:!1});else return b&&this.set(c,this.keys.sign(a),{signed:!1}),d}}}set(a,b,c={}){const d=this.a;var e=this.request;let f=d.getHeader("Set-Cookie")||[];"string"==typeof f&&(f=[f]);var g=e.protocol;e=e.connection.encrypted;g=void 0!==this.secure?!!this.secure:"https"==g||e;const {signed:k=!!this.keys,...h}=c;a=new ob(a,b,h);if(!g&&c.secure)throw Error("Cannot send secure cookie over unencrypted connection");a.secure=g;"secure"in c||(a.secure=g);qb(f,a);if(c&&k){if(!this.keys)throw Error(".keys required for signed cookies");
a.value=this.keys.sign(a.toString());a.name+=".sig";qb(f,a)}(d.set?sa.prototype.setHeader:d.setHeader).call(d,"Set-Cookie",f);return this}}function qb(a,b){if(b.overwrite)for(var c=a.length-1;0<=c;c--)0===a[c].indexOf(b.name+"=")&&a.splice(c,1);a.push(nb(b))};const rb=/^\s*([^\s;]+)\s*(?:;(.*))?$/;function sb(a){return a.split(",").map((b,c)=>{var d=rb.exec(b.trim());if(d){b=d[1];var e=1;if(d[2]){d=d[2].split(";");for(let f=0;f<d.length;f++){const g=d[f].trim().split("=");if("q"==g[0]){e=parseFloat(g[1]);break}}}c={charset:b,q:e,h:c}}else c=null;if(c)return c}).filter(Boolean)}
